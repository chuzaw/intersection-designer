...
RG.prototype.refuge=function(cd,cw,rw){if(rw<=0)return null;var hw=rw*0.5;return[this.pa(-hw,cd),this.pa(hw,cd),this.pa(hw,cd+cw),this.pa(-hw,cd+cw)]};
RG.prototype.twoGap=function(cd,cw){var gw=LW*0.4;return[this.pa(-gw,cd),this.pa(gw,cd),this.pa(gw,cd+cw),this.pa(-gw,cd+cw)]};
RG.prototype.chan=function(){var cl=LW,hw=MW/2+this.inW;return[this.pa(hw,0),this.pa(hw-LW,LW*0.2),this.pa(hw,cl)]};
RG.prototype.label=function(d){var p=this.dir.scale(d);return{x:p.x,y:p.y}};
...
var cw=3,bw=.45,bg=.45,centerGap=2.5,stopGap=1.5,arrowGap=2,al=4,ts=cc&&cc.twoStage,refW=MW+.4;
...
var nX=arm.crosswalk,nR=arm.refugeIsland||ts;
...
for(var j=0;j<arm.outLanes-1;j++){var o=rg.pa(rg.outDiv(j),sd);out.divs.push({start:o,end:rg.pa(rg.outDiv(j),L)})}
for(var j=0;j<arm.inLanes-1;j++){var o=rg.pa(rg.inDiv(j),sd);out.divs.push({start:o,end:rg.pa(rg.inDiv(j),L)})}
...
if(nX){var xs=rg.xings(cd,cw,bw,bg);for(var si=0;si<xs.length;si++)out.xings.push(xs[si]);if(ts)out.islands.push({pts:rg.twoGap(cd,cw),type:"gap"});if(nR){var ri2=rg.refuge(cd,cw,refW);if(ri2)out.islands.push({pts:ri2,type:"refuge"})}}
for(var li2=0;li2<arm.inLanes;li2++){var at=(arm.inLaneArrows||[])[li2];if(!at){var d=defA(arm.inLanes)[li2];at=d?d.a:""}if(!at)continue;out.arrows.push(rg.inArrow(ad,al,li2,at))}
var lp=rg.label(L*0.85);out.labels.push({x:lp.x,y:lp.y,text:arm.name})}
...
for(var i=0;i<out.stops.length;i++){p.push(out.stops[i].start,out.stops[i].end)}return p};
return out};
