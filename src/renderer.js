...
var m=100,sw=(mxX-mnX)*sc+2*m,sh=(mxY-mnY)*sc+2*m,cx=m-mnX*sc,cy=m+mxY*sc;
...
o+="<style>.rd{fill:#6b7280;stroke:#4b5563;stroke-width:1.5}.ct{fill:#6b7280;stroke:none}.cut{fill:#6b7280;stroke:none}.ln{stroke:#e5e7eb;stroke-width:1.8;stroke-linecap:round}.dy{stroke:#fbbf24;stroke-width:2.5}.stk{stroke:#fff;stroke-width:4}.cr{fill:#e5e7eb;stroke:#d1d5db;stroke-width:.8}.ri{fill:#fbbf24;stroke:#d99a0b;stroke-width:1.2}.lb{font:96px system-ui;font-weight:500;fill:#333;text-anchor:middle;dominant-baseline:central}</style>";
o+="<path class=\"rd\" d=\""+surfacePath(out.roads,1.2,out.smoothConnection)+"\"/>"
;for(var i=0;i<out.divs.length;i++){var d=out.divs[i],dash=d.type=="out"?"150 120":"6 6";o+="<line class=\"ln\" stroke-dasharray=\""+dash+"\""+seg(d.start,d.end)+"/>"}
...
for(var i=0;i<out.islands.length;i++){var s=out.islands[i];if(s.type=="gap"){o+="<polygon class=\"cut\" points=\""+pts(s.pts)+"\"/>"}else{o+="<polygon class=\"ri\" points=\""+pts(s.pts)+"\"/>"}}
...
for(var i=0;i<out.labels.length;i++){var l=out.labels[i];if(!l.text)continue;var x=sx(l.x),y=sy(l.y);o+="<text class=\"lb\" x=\""+x+"\" y=\""+y+"\" transform=\"rotate("+l.angle.toFixed(2)+" "+x+" "+y+")\">"+esc(l.text)+"</text>"}
o+="</svg>";return o}
w.render=render;
})(this);
